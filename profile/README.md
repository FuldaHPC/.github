...
| `misc`      | `misc-<slug>`                              | `misc-meeting-log-template`        | optional                                                 |
...
```
paper | artifact | benchmark | talk | poster | workshop | proposal | grant | tool | topic | teaching | infra | protocol | cv | travel | misc
```
...
Template: [`misc-meeting-log-template`](https://github.com/FuldaHPC/misc-meeting-log-template)

`paper-*` and `teaching-*` repositories use this template unless a documented reason says otherwise.

## 8. AI Coding Assistants

Shared rules for AI coding assistants (e.g., Claude Code, Cursor) live in [`misc-ai-rules`](https://github.com/FuldaHPC/misc-ai-rules). Reference them in your editor to keep language, style, and editing conventions consistent across projects.
